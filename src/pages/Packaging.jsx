...
export default function Packaging() {
  ...
  const [form, setForm] = useState({ name: "", type: "box", unit_cost: 0, stock_quantity: 0 });
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["packaging"] }); setOpen(false); resetForm(); toast.success("Item added"); },
  });

  const updateMut = useMutation({
    mutationFn: ({ id, data }) => base44.entities.PackagingItem.update(id, data),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["packaging"] }); setOpen(false); resetForm(); toast.success("Item updated"); },
  });

  const deleteMut = useMutation({
    mutationFn: (id) => base44.entities.PackagingItem.delete(id),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["packaging"] }); toast.success("Deleted"); },
  });

  const resetForm = () => { setForm({ name: "", type: "box", unit_cost: 0, stock_quantity: 0 }); setEditId(null); };

  const handleEdit = (item) => {
    setForm({ name: item.name, type: item.type || "box", unit_cost: item.unit_cost || 0, stock_quantity: item.stock_quantity || 0 });
    setEditId(item.id);
    setOpen(true);
  };
      ...
      <div className="flex items-center justify-between">
        ...
        <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) resetForm(); }}>
          ...
          <DialogContent className="bg-[hsl(224,20%,10%)] border-slate-700/50 text-white rounded-2xl max-w-sm">
            <DialogHeader><DialogTitle className="text-white">{editId ? "Edit" : "Add"} Packaging</DialogTitle></DialogHeader>
              ...
              <div><Label className="text-xs text-slate-400">Type</Label>
                <Select value={form.type} onValueChange={(v) => setForm({ ...form, type: v })}>
                  ...
                  <SelectContent className="bg-slate-800 border-slate-700">
                    {["box", "bag", "wrap", "padding", "label", "other"].map((t) => <SelectItem key={t} value={t} className="text-slate-200 capitalize">{t}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
          ...
          <TableBody>
            {items.map((item) => (
              <TableRow key={item.id} className="border-white/[0.04] hover:bg-white/[0.02]">
                ...
                <TableCell className="text-slate-400 text-sm capitalize">{item.type}</TableCell>
            ...
            {items.length === 0 && (
              <TableRow><TableCell colSpan={5} className="text-center py-10"><Package className="w-6 h-6 text-slate-700 mx-auto mb-2" /><p className="text-xs text-slate-600">No packaging items</p></TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      </motion.div>
